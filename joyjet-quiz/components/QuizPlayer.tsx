...
export default function QuizPage() {
  ...
  if (!quiz) return <p className="p-6 text-center text-red-500 text-lg font-semibold">Quiz not found</p>;
        ...
        <div className="grid grid-cols-1 gap-4">
          {quiz.questions.map((q, i) => {
            ...
            return (
              <div
                key={i}
                className={`p-4 rounded shadow ${correct ? "bg-(--success)" : "bg-(--danger)"} text-left relative`}
              >
                ...
                {correct && (
                  <span className="absolute right-2 top-2 text-green-600 text-2xl">&#10003;</span>
                )}
                {!correct && selected && (
                  <span className="absolute right-2 top-2 text-red-600 text-2xl">&#10007;</span>
                )}
              </div>
            );
          })}
  ...
  return (
    <div className="max-w-3xl mx-auto p-6 bg-(--card-bg) rounded shadow text-(--text-color)">
      ...
      <div className="w-full h-3 bg-(--foreground)/20 rounded mb-6 relative">
        ...
        />
        <div
          className="absolute -top-1.5 left-[calc(progressPercent%)] w-5 h-5 bg-(--primary) border-2 border-(--background) rounded-full transform -translate-x-1/2"
        />
      </div>

      {/* Question card */}
      <div className="p-4 border rounded shadow mb-4 bg-(--background) relative">
          ...
          {question.answers.map(a => {
            ...
            return (
              <div key={a.id} className="relative">
                ...
                {/* Animated bullet */}
                {selected && (
                  <span className="absolute left-3 top-1/2 transform -translate-y-1/2 w-4 h-4 bg-(--primary) rounded-full animate-bounce"></span>
                )}
              </div>
            );
          })}
